...
public class RollingTimeCount {
     ...
     */
    public RollingTimeCount(long timeWindowMillis) {
    ...
    public void increment() {
        if (firstTime == null) {
            firstTime = new TimeCount(1d);
        } else {
            deque.addLast(new TimeCount(1d));
        }
        total += 1d;
    }
    ...
    public int getCount() {
        ...
        if (this.deque.isEmpty() || this.firstTime.time >= currentWindowStart) {
            result = 1000d * total / (now - this.firstTime.time);
        } else {
